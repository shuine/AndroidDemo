 ...
 */
package com.shine.demo.viewpager.smartTabLayout;
...
class SmartTabStrip extends LinearLayout {
  ...
  private void drawDecoration(Canvas canvas) {
    ...
    if (tabCount > 0) {
      ...
      //====
      int top = Utils.getTop(selectedTab,indicatorWithoutPadding);
      int bottom = Utils.getBottom(selectedTab,indicatorWithoutPadding);
      ...
      if (selectionOffset > 0f && selectedPosition < (getChildCount() - 1)) {
        ...
        int nextTop = Utils.getTop(nextTab,indicatorWithoutPadding);
        int nextBottom = Utils.getBottom(nextTab,indicatorWithoutPadding);

        top = (int) (startOffset * nextTop + (1.0f - startOffset) * left);
        bottom = (int) (endOffset * nextBottom + (1.0f - endOffset) * right);
...
//    canvas.drawRect(left, height - bottomBorderThickness, right, height, borderPaint);
    canvas.drawRect(getWidth(), 0, height, getHeight(), borderPaint);
  }
